  ...
  <div class="customer-layout">
    ...
    <main class="customer-main">
      <header class="customer-header">
        <h1 class="customer-title">Замечания и нарушения</h1>
      </header>
      ...
      <section class="issues-section">
        <div class="section-header">
          ...
          <button type="button" class="upload-btn" @click="showCreateModal = true">
            + Создать замечание
          </button>
        </div>
...
const filteredIssues = computed(() => {
  ...
  switch (activeFilter.value) {
    case 'remark': return items.filter((i: any) => i.type === 'remark')
...
// ✅ Логика группировки по объектам
const groupedIssues = computed(() => {
  const map = new Map<number, any[]>()
  for (const issue of filteredIssues.value) {
    if (!map.has(issue.object_id)) map.set(issue.object_id, [])
    map.get(issue.object_id)!.push(issue)
  }

  const result = []
  for (const [objectId, issues] of map.entries()) {
    const obj = objects.value.find(o => o.id === objectId)
    result.push({ id: objectId, name: obj?.name || `Объект #${objectId}`, issues })
  }
  return result
})
...
async function loadData() {
  ...
  try {
    ...
    const [objRes, issRes] = await Promise.all([
      fetch(`${API_BASE}/customer/dashboard/objects`, { headers }),
      fetch(`${API_BASE}/customer/issues`, { headers })
    ])
...
async function handleCreate(payload: any) {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/customer/objects/${payload.object_id}/issues`, {
...
function openIssue(issue: any) {
  router.push({ name: 'customer-object-details', params: { id: issue.object_id } })
}
