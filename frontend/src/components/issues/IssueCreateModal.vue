<template>
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-card">
      ...
      <div v-if="!props.presetObjectId" class="form-field">
        <label>Объект <span class="required">*</span></label>
        <select v-model.number="form.object_id" required>
      ...
      <div class="form-field">
        ...
        <input v-model="form.title" type="text" placeholder="Краткий заголовок" />
      </div>

      <div v-if="mode === 'violation'" class="form-field">
        <label>Классификатор нарушения <span class="required">*</span></label>
        <input v-model="form.classifier_code" type="text" placeholder="Например: KO-12.4" />
      </div>
      ...
      <div class="form-field">
        ...
        <div v-if="selectedFiles.length" class="files-list">
          <div v-for="file in selectedFiles" :key="fileKey(file)" class="file-chip">
            ...
            <button type="button" class="remove-file-btn" @click="removeFile(file)">×</button>
          </div>
        </div>
      </div>
...
watch(
  () => [props.mode, props.presetObjectId],
  () => resetForm(),
  { immediate: true },
)

function resetForm() {
  form.object_id = props.presetObjectId ?? null
  form.title = ''
  form.description = ''
  form.due_date = ''
...
function submit() {
  ...
  if (!form.object_id) {
    localError.value = props.presetObjectId ? 'Ошибка:Выберите объект' : 'Выберите объект'
    return
  }
...
<style scoped>
.modal-overlay { position: fixed; inset: 0; background: rgba(15, 23, 42, 0.55); display: flex; justify-content: center; align-items: center; padding: 16px; z-index: 60; }
.modal-card { width: 100%; max-width: 560px; background: #fff; border-radius: 16px; padding: 22px 24px 20px; box-shadow: 0 20px 50px rgba(15, 23, 42, 0.25); box-sizing: border-box; max-height: calc(100vh - 32px); overflow-y: auto; }
...
.form-field input, .form-field textarea, .form-field select { border-radius: 10px; border: 1px solid #d1d5db; padding: 8px 10px; font-size: 14px; outline: none; background: #f9fafb; }
.form-field textarea { resize: vertical; min-height: 84px; }
.form-field select { cursor: pointer; }
.required { color: #dc2626; }
.files-list { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 4px; }
.file-chip { display: inline-flex; align-items: center; gap: 8px; background: #eef2ff; color: #3730a3; border-radius: 999px; padding: 6px 10px; font-size: 12px; max-width: 100%; }
.file-name { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.remove-file-btn { border: none; background: transparent; color: #4338ca; cursor: pointer; font-size: 14px; line-height: 1; padding: 0; }
.modal-actions { display: flex; justify-content: flex-end; gap: 8px; margin-top: 4px; }
.primary-btn, .secondary-btn { padding: 8px 16px; border-radius: 999px; border: none; font-size: 14px; cursor: pointer; }
.primary-btn { background: #4f46e5; color: #fff; }
.primary-btn:disabled { opacity: 0.5; cursor: default; }
.secondary-btn { background: #e5e7eb; color: #374151; }
.state { font-size: 13px; color: #6b7280; }
